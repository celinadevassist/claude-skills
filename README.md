...
| [deployment-setup](./deployment-setup/) | Deploy onto an **existing** Portainer + Nginx-Proxy-Manager host: Dockerfile, GitHub Actions CI/CD, Portainer stack config. Sibling of `hetzner-monolith-deploy` (new dedicated server) and `single-host-systemd-deploy` (shared box, no Docker) |
| [hetzner-monolith-deploy](./hetzner-monolith-deploy/) | Stand up a **new dedicated** Hetzner/DO single server — Caddy auto-HTTPS, GH Actions multi-arch + Watchtower push-to-deploy, Prisma migrate-on-container-start, nightly local + Backblaze B2 backups, MXroute SMTP. The "Follow-Up Expenses" blueprint |
...
| [project-readiness-report](./project-readiness-report/) | Rolls up the three project side-cars (`.project-meta.json` + `.project-skills.json` + `.project-technical-prd.json`) into a single 0-100 readiness score + tier + tile-friendly headline + prioritized punch-list. Designed to be the one number Mission Control's portfolio view shows per project |
| [project-skill-advisor](./project-skill-advisor/) | Recommends which skills in this repo to apply to a given project, in dependency-correct order. Cross-references the project against [`skills-graph.yml`](./skills-graph.yml); emits a `.project-skills.json` side-car with four buckets (recommendations / alreadyApplied / notRecommended / catalogGaps) plus `$findings.warnings[]` (ordering/prereq/stale) and per-entry `nextActions[]` for Mission Control's apply drawer to consume |
...
| [realtime-feed-sse-or-socketio](./realtime-feed-sse-or-socketio/) | Real-time server→client push for NestJS + React — replaces "click Refresh" with a live `● LIVE` ticker. Defaults to SSE (zero deps, native `EventSource`, auto-reconnect); falls back to Socket.IO when bidirectional or explicit room semantics are required. Ships the `RxJS Subject` bus, `@Sse()` controller with hello+heartbeat+tenant-filtered live merge, JWT-strategy `?token=` fallback (EventSource can't set headers), and the React `useEffect` with dedup + memory cap + LIVE/OFFLINE chip |
| [responsive-layout-discipline](./responsive-layout-discipline/) | Mantine breakpoint discipline so pages don't break at 390px: responsive `Grid.Col` / `SimpleGrid` spans, `visibleFrom`/`hiddenFrom` for header chips, `ScrollArea` for terminals + wide tables, no fixed pixel widths. Pairs with `mantine-theme-discipline` as a 4-grep gate |
| [single-host-systemd-deploy](./single-host-systemd-deploy/) | Add a project to an **existing shared host** running Caddy + sibling projects — no Docker, systemd unit (prod + dev) + Caddy reverse-proxy block + sslip.io subdomain + `scripts/deploy.sh` + GitHub Actions CD over SSH. The mission-control / ems / cartflow / idea-keep pattern |
...
| [skill-verification-runner](./skill-verification-runner/) | Executes `nextActions[]` from `.project-skills.json` and stamps `verified`/`verifiedAt`/`verificationOutput` on every applied skill. The runtime half of advisor v2 — without it every project's `skillVerification` dim in `.project-readiness.json` stays at 0/15. Ships a stdlib-only Python runner at [`scripts/run-skill-verification.py`](./scripts/run-skill-verification.py) |
